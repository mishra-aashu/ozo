...
const LocationPicker = ({ isOpen, onClose }) => {
  ...
  }, [isOpen, fetchUserAddresses, fetchHierarchicalData])
  ...
  const dbMatches = React.useMemo(() => {
    ...
    const matches = []
    const nearestCity = useLocationStore.getState().nearestCity
    const nearestCityName = nearestCity?.name ? nearestCity.name.split(',')[0].trim() : ''
                      ...
                      {filteredAddresses.map((addr) => {
                        ...
                        return (
                          <div
                            ...
                            className={`address-card group ${addr.is_default ? 'address-card-selected' : ''}`}
                          >
                            {/* Left Side Icon */}
                  ...
                  <div className="relative w-[75vw] h-[75vw] max-w-[340px] max-h-[340px] opacity-85 dark:opacity-75 transition-opacity duration-300 mt-20">
                    ...
                    >
                      <defs>
                        ...
                        <radialGradient id="globe-shading-picker" cx="30%" cy="30%" r="70%">
                        ...
                        <radialGradient id="globe-halo-picker" cx="50%" cy="50%" r="50%">
                      ...
                      <circle cx="180" cy="90" r="92" fill="url(#globe-halo-picker)" />
                      ...
                      <clipPath id="globe-clip-picker">
                        <circle cx="180" cy="90" r="85" />
                      </clipPath>

                      {/* Rotating Landmasses */}
                      <g clipPath="url(#globe-clip-picker)">
                        <g>
                          {/* First Map Copy */}
                      ...
                      <circle cx="180" cy="90" r="85" fill="url(#globe-shading-picker)" pointerEvents="none" />

                      {/* Spherical Graticule Grid Lines (Latitudes/Longitudes Overlay) */}
                      <g clipPath="url(#globe-clip-picker)" pointerEvents="none">
                        {/* Latitudes */}
