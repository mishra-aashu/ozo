-- Migration: OneSignal push notification system setup & database notification lifecycle trigger
-- Date: 2026-06-14

-- 1. Add onesignal_subscription_id column to users table
ALTER TABLE public.users ADD COLUMN IF NOT EXISTS onesignal_subscription_id TEXT;

-- 2. Create the notification lifecycle trigger function on public.orders
...
DECLARE
  ...
  v_payout NUMERIC;
BEGIN
  -- 1. ADMIN ALERT: Customer places a new order (INSERT on orders table with status = 'pending')
