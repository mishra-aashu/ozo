-- Migration: Replace raw Product UUIDs in error exceptions with Product Names in create_order_secure
-- Created: 2026-07-28
...
CREATE OR REPLACE FUNCTION public.create_order_secure(
    ...
    p_charity_donation NUMERIC DEFAULT 0.00
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    LOOP
        ...
        -- Fetch product name to provide user-friendly error messages
        SELECT name INTO v_product_name FROM public.products WHERE id = v_product_id;

        IF v_qty <= 0 THEN
            RAISE EXCEPTION 'Invalid quantity % for item "%"', v_qty, COALESCE(v_product_name, 'Selected Item');
        END IF;
        ...
        IF v_mart_id IS NOT NULL THEN
            ...
            IF NOT FOUND THEN
                RAISE EXCEPTION 'Product "%" is not available in the selected mart.', COALESCE(v_product_name, 'Item');
            END IF;

            IF NOT v_is_available OR v_avail_qty < v_qty THEN
                RAISE EXCEPTION 'Insufficient stock for "%" in store. Available: %, Requested: %', COALESCE(v_product_name, 'Item'), COALESCE(v_avail_qty, 0), v_qty;
            END IF;
        ...
        ELSE
            ...
            IF NOT v_is_available OR v_avail_qty < v_qty THEN
                RAISE EXCEPTION 'Insufficient stock for "%". Available: %, Requested: %', COALESCE(v_product_name, 'Item'), COALESCE(v_avail_qty, 0), v_qty;
            END IF;
...
GRANT EXECUTE ON FUNCTION public.create_order_secure(
    UUID, NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT, TEXT, TEXT, TEXT, TIMESTAMPTZ, TEXT, NUMERIC, NUMERIC, JSONB, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, TEXT, UUID, NUMERIC, NUMERIC, NUMERIC
